...
#         continue
    ...
    for j in range(elementCount):
        try:
            ...
            reviewScore = int(ratingDiv.find_element_by_xpath(".//span[contains(@class, 'ui_bubble_rating bubble_')]").get_attribute(
                "class").split("_")[3])
            
            ratingGeneral = review[j].find_element_by_xpath(".//div[contains(@class, 'ui_poi_review_rating')]")
            ratingGeneralScore = int(ratingGeneral.find_element_by_xpath(".//span[contains(@class, 'ui_bubble_rating bubble_')]").get_attribute(
                    "class").split("_")[3])
            
#             print(ratingGeneralScore)
            
            linkReviewLocation = review[j].find_element_by_xpath(".//div[contains(@class, '_2X5tM2jP _2RdXRsdL _1gafur1D')]")
            linkReviewLocation = linkReviewLocation.find_element_by_xpath(".//a[contains(@class, '')]").get_attribute('href')
...
usersDF = pd.read_csv('/mnt/authfiles/user_profiles/users_with_demo.csv')
...
#     print(username)
    if check == False:
        print('user: ', i)
        print(username)
        filename = '/mnt/authfiles/user_profiles/' + str(username) + '.csv'
...
#         x = profiles.insert_one(user_profile)
        ...
        usersDF.to_csv('/mnt/authfiles/user_profiles/users_with_demo.csv', index=False)
        driver.close()    
